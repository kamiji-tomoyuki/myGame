...
Microsoft::WRL::ComPtr<ID3D12RootSignature> PipeLineManager::CreateRootSignature(Microsoft::WRL::ComPtr<ID3D12RootSignature> rootSignature) {
    ...
    // DescriptorRange for regular texture (t0)
    ...
    // DescriptorRange for environment texture (t1)
    D3D12_DESCRIPTOR_RANGE descriptorRangeEnvironment[1] = {};
    descriptorRangeEnvironment[0].BaseShaderRegister = 1;                                                   // レジスタ番号1から始まる
    descriptorRangeEnvironment[0].NumDescriptors = 1;                                                       // 数は1つ
    descriptorRangeEnvironment[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;                              // SRVを使う
    ...
    rootParameters[0].Descriptor.ShaderRegister = 0;                                              // レジスタ番号0とバインド (Material)
    rootParameters[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;                              // CBVをつかう
    rootParameters[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;                          // VertexShaderで使う
    rootParameters[1].Descriptor.ShaderRegister = 0;                                              // レジスタ番号0とバインド (Transform)
    rootParameters[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;                 // DescriptorTableを使う
    rootParameters[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;                           // PixelShaderで使う
    rootParameters[2].DescriptorTable.pDescriptorRanges = descriptorRange;                        // Tableの中身の配列を指定 (gTexture t0)
    rootParameters[2].DescriptorTable.NumDescriptorRanges = _countof(descriptorRange);            // Tableで利用する数
    rootParameters[3].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;                              // CBVを使う
    rootParameters[3].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;                           // PixelShaderで使う
    rootParameters[3].Descriptor.ShaderRegister = 1;                                              // レジスタ番号1とバインド (DirectionalLight)
    rootParameters[4].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;                              // CBVを使う
    rootParameters[4].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;                           // PixelShaderで使う
    rootParameters[4].Descriptor.ShaderRegister = 2;                                              // レジスタ番号2とバインド (Camera)
    rootParameters[5].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;                              // CBVを使う
    rootParameters[5].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;                           // PixelShaderで使う
    rootParameters[5].Descriptor.ShaderRegister = 3;                                              // レジスタ番号3とバインド (PointLight)
    rootParameters[6].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;                 // DescriptorTableを使う
    rootParameters[6].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;                           // PixelShaderで使う
    rootParameters[6].DescriptorTable.pDescriptorRanges = descriptorRangeEnvironment;             // Tableの中身の配列を指定 (gEnvironmentTexture t1)
    rootParameters[6].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeEnvironment); // Tableで利用する数
...
Microsoft::WRL::ComPtr<ID3D12PipelineState> PipeLineManager::CreateGraphicsPipeLine(Microsoft::WRL::ComPtr<ID3D12PipelineState> graphicsPipelineState, Microsoft::WRL::ComPtr<ID3D12RootSignature> rootSignature, BlendMode blendMode_) {
    ...
    // InputLayout - Object3d.PS.hlslで期待される頂点属性
    ...
    // NORMAL (float3) - 環境マッピングとライティングに必要
    ...
    rasterizerDesc.CullMode = D3D12_CULL_MODE_NONE;  // カリングなし（環境マッピング用）
    ...
    depthStencilDesc.DepthFunc = D3D12_COMPARISON_FUNC_LESS_EQUAL; // 近いものを描画

    // GraphicsPipelineState設定
...
Microsoft::WRL::ComPtr<ID3D12RootSignature> PipeLineManager::CreateSkinningRootSignature(Microsoft::WRL::ComPtr<ID3D12RootSignature> rootSignature) {
    ...
    rootParameters[6].DescriptorTable.pDescriptorRanges = descriptorRangeEnvironment;             // Tableの中身の配列を指定 (gEnvironmentTexture t1)
    rootParameters[6].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeEnvironment); // Tableで利用する数
    rootParameters[7].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;      // DescriptorTableを使う (スキニング用)
...
Microsoft::WRL::ComPtr<ID3D12PipelineState> PipeLineManager::CreateSkinningGraphicsPipeLine(Microsoft::WRL::ComPtr<ID3D12PipelineState> graphicsPipelineState, Microsoft::WRL::ComPtr<ID3D12RootSignature> rootSignature) {
    ...
    // InputLayout - スキニング用にWEIGHTとINDEXを追加
    ...
    inputElementDescs[3].InputSlot = 1;         // 二番目のVBVスロット
    inputElementDescs[3].AlignedByteOffset = 0; // 新しいスロットなので0から開始
    inputElementDescs[4].SemanticName = "INDEX";
    inputElementDescs[4].SemanticIndex = 0;
    inputElementDescs[4].Format = DXGI_FORMAT_R32G32B32A32_SINT;
    inputElementDescs[4].InputSlot = 1; // 同じスロット
